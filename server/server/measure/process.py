...
def get_pixel_per_cm(cnts, dist_in_cm):
...
def draw_remaining_contours(cnts, image, pixel_per_cm):
    ...
    for cnt in cnts:
        ...
        cv2.putText(
            image, 
            "{:.1f}cm".format(wid), (int(mid_pt_horizontal[0] - 15), 
...
def process_image(img_path: str, vertical: int, area_threshold):
    ...
    # Get pixel per cm
    pixel_per_cm = get_pixel_per_cm(cnts, vertical)

    # Draw remaining contours
    image = draw_remaining_contours(cnts, image, pixel_per_cm)
